...
class CNNEncoder(nn.Module):
    def __init__(self, in_channels, kernel_sizes, features, strides, maxpools, dropouts, deforms) -> Any:
        ...
        assert len(kernel_sizes) == len(strides) == len(features), "kernel_sizes, features, and strides must have the same length"
...
class CNNDecoder(nn.Module):
    def __init__(self, 
        ...
        assert len(cnn_kernel_sizes) == len(tcv_strides) == len(features), "cnn_kernel_sizes, features, and tcv_strides must have the same length"
...
class Model_Base(nn.Module):
    def __init__(self, spatial_shapes, do_ds=False, in_channels=4, out_channels=3,
        ...
        # check dropouts
        cnn_dropouts     = [cnn_dropouts     for _ in spatial_shapes] if not isinstance(cnn_dropouts    , list) else cnn_dropouts
        hyb_cnn_dropouts = [hyb_cnn_dropouts for _ in spatial_shapes] if not isinstance(hyb_cnn_dropouts, list) else hyb_cnn_dropouts
        hyb_tf_dropouts  = [hyb_tf_dropouts  for _ in spatial_shapes] if not isinstance(hyb_tf_dropouts , list) else hyb_tf_dropouts
        
        # check strides
        cnn_strides = [[st for _ in spatial_shapes] if not isinstance(st, list) else st for st in cnn_strides]
        hyb_strides = [[st for _ in spatial_shapes] if not isinstance(st, list) else st for st in hyb_strides]
        ...
        for stride in cnn_strides+hyb_strides:
            enc_spatial_shaps.append([int(np.ceil(ss/st)) for ss,st in zip(enc_spatial_shaps[-1], stride)])
        dec_spatial_shaps = [enc_spatial_shaps[-1]]
        for stride in hyb_strides[::-1]+cnn_strides[::-1]:
            dec_spatial_shaps.append([int(np.ceil(ss*st)) for ss,st in zip(dec_spatial_shaps[-1], stride)])
...
class MultiScaleLKA3DModule(nn.Module):
    def __init__(self, in_channels=256):
        ...
        paddings = [1, 4, 9]
        dilations = [1, 2, 3]
        self.conv0 = nn.Conv3d(in_channels, in_channels, kernel_size=5, padding=2, groups=in_channels)
        self.spatial_convs = nn.ModuleList([nn.Conv3d(in_channels, in_channels, kernel_size=kernels[i], stride=1,
...
class BridgeModule(nn.Module):
    def __init__(self, feats:list[int], c_attn_block:nn.Module, s_attn_block:nn.Module, mlk_attn_block:nn.Module):
        ...
        for f1 in feats:
            for f2 in feats:
                self.projs[f"{f1}->{f2}"] = nn.Conv3d(f1, f2, kernel_size=1, padding=0) if f1 != f2 else nn.Identity()

...
def partition(image, div_num):
    ...
    for label_temp in label:
        index_temp = label2order[label_temp]
        index_vec_temp_1 = np.hstack(
            (label_index_vec[index_temp[0]], label_index_vec[index_temp[1]])
        )
        index_vec_temp_2 = np.hstack(
            (label_index_vec[index_temp[2]], label_index_vec[index_temp[3]])
        )
...
def cut_partition(image, div_num):
    ...
    ori_cut_partition = image[
        :,
        :,
        h_start : h_start + cut_size[0],
        w_start : w_start + cut_size[1],
        d_start : d_start + cut_size[2],
    ]
    image[
        :,
        :,
